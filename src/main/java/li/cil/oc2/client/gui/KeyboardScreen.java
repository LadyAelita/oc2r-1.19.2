...
public final class KeyboardScreen extends Screen {
    ...
    private void blitQuad(final PoseStack stack, final int x0, final int y0, final int x1, final int y1, final int color) {
        ...
        builder.vertex(stack.last().pose(), x0, y1, 0).color(color).endVertex();
        builder.vertex(stack.last().pose(), x1, y1, 0).color(color).endVertex();
        builder.vertex(stack.last().pose(), x1, y0, 0).color(color).endVertex();
        builder.vertex(stack.last().pose(), x0, y0, 0).color(color).endVertex();
        tesselator.end();
    }
